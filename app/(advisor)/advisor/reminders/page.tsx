...
export default async function AdvisorRemindersPage() {
	...
	const myCustomerIds = new Set((customers ?? []).filter((c: any) => c.advisor_id === advisorId).map((c: any) => c.id));
	const filtered = (reminders ?? []).filter((r: any) => !r.customer_id || myCustomerIds.has(r.customer_id));
					...
					<div className="grid grid-cols-1 gap-4">
						{pending.length === 0 ? (
							<div className="py-12 text-center bg-zinc-50/50 rounded-xl border border-dashed border-zinc-200">
								<p className="text-sm text-zinc-400 italic">No pending reminders</p>
							</div>
						) : (
