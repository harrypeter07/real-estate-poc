    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 rounded-2xl border border-zinc-200/80 bg-white shadow-[0_4px_25px_rgba(0,0,0,0.015)]">
        <div className="flex items-center gap-4">
          <Link href="/customers" className="p-2.5 rounded-xl border border-zinc-200 bg-white text-zinc-500 hover:text-zinc-800 hover:bg-zinc-50 transition-colors shadow-sm active:scale-95">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-zinc-400 font-bold uppercase tracking-wider font-mono">Customer Account Details</p>
            </div>
          </div>
        </div>
                      ...
                      <div
                        ...
                        className={cn(
                          ...
                          isFullyPaid 
                            ...
                            : isOverdue 
                              ? "border-l-red-500 border-red-100 bg-red-50/15" 
                              : "border-l-teal-500"
                        )}
                      >
                        <div className="flex items-center justify-between gap-3 border-b border-zinc-100 pb-2">
                          ...
                          {isFullyPaid ? (
                            ...
                            <Badge className="bg-red-50 text-red-700 border border-red-200/50 text-[9px] font-black uppercase px-2 py-0.5 rounded-full shrink-0 flex items-center gap-1 animate-pulse shadow-[0_2px_8px_rgba(239,68,68,0.06)]">
                        ...
                        <div className="space-y-1.5">
                          ...
                          {lastPayment && (
                            <div className="flex items-center justify-between gap-3 text-[11px] border-t border-dashed border-zinc-100 pt-2 mt-2">
                              <span className="text-zinc-400 font-bold">Last Payment ({formatDate(lastPayment.payment_date)})</span>
                              <span className="font-black text-emerald-650 bg-emerald-50/50 px-2 py-0.5 rounded-lg border border-emerald-100/50">{formatCurrency(lastPayment.amount)}</span>
                            </div>
                          )}

                          {nextDue && !isFullyPaid && (
                            <div className="flex items-center justify-between gap-3 text-[11px] pt-1">
                              <span className="text-zinc-400 font-bold">Next EMI Due</span>
                              <span className={cn("font-black", isOverdue ? "text-red-650" : "text-zinc-700")}>
                                {formatDate(nextDue)}
                              </span>
                            </div>
                          )}
