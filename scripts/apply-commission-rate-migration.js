...
async function applyMigration() {
	...
	if (!dbPassword) {
		...
		console.log("Please run the SQL statements inside 'supabase/migrations/20260511000000_advisor_project_commissions_commission_rate.sql' in the Supabase SQL Editor, or set DB_PASSWORD in your .env file and run this script again.");
		process.exit(1);
	}
	...
	try {
		const migrationPath = path.join(__dirname, "..", "supabase", "migrations", "20260511000000_advisor_project_commissions_commission_rate.sql");
		console.log(`Reading migration from ${migrationPath}...`);
		const sql = fs.readFileSync(migrationPath, "utf8");

		console.log("Executing commission rate SQL migration...");
