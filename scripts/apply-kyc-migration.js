...
async function applyMigration() {
	...
	if (!dbPassword) {
		...
		console.log("Please run the SQL statements inside 'supabase/migrations/20260525000000_customer_kyc_columns_and_policies.sql' in the Supabase SQL Editor, or set DB_PASSWORD in your .env file and run this script again.");
		process.exit(1);
	}
	...
	try {
		const migrationPath = path.join(__dirname, "..", "supabase", "migrations", "20260525000000_customer_kyc_columns_and_policies.sql");
		console.log(`Reading migration from ${migrationPath}...`);
		const sql = fs.readFileSync(migrationPath, "utf8");

		console.log("Executing KYC SQL migration...");
