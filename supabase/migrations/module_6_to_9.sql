-- ============================================================
-- FIXED SQL — Modules 6-9 Upgrade
-- Removed all GENERATED ALWAYS AS with CURRENT_DATE (not immutable)
-- remaining_amount and overdue_days handled via triggers instead
-- ============================================================
...
-- Migrate existing enquiry_status → pipeline_stage
...
-- ============================================================
-- STEP 5: emi_schedule
-- FIX: removed GENERATED ALWAYS AS for remaining_amount and overdue_days
--      Both are now plain columns updated by triggers
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.emi_schedule (
  ...
  remaining_amount NUMERIC(12,2) NOT NULL DEFAULT 0,  -- updated by trigger
  ...
  waiver_reason    TEXT,
  overdue_days     INTEGER NOT NULL DEFAULT 0,         -- updated by trigger + daily cron
  notes            TEXT,
  created_at       TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at       TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
-- 8a: Generic updated_at trigger function
...
-- 8b: EMI trigger — auto-calc remaining_amount + overdue_days on INSERT/UPDATE
--     This replaces the broken GENERATED ALWAYS AS columns
CREATE OR REPLACE FUNCTION fn_emi_calc_fields()
RETURNS TRIGGER AS $$
BEGIN
  -- remaining_amount = emi_amount - paid_amount (never below 0)
  NEW.remaining_amount := GREATEST(NEW.emi_amount - NEW.paid_amount, 0);
 
  -- overdue_days: only when pending/partial and past due date
  ...
  -- Auto-escalate status to 'overdue' if past due and not paid
...
DROP TRIGGER IF EXISTS trg_emi_calc_fields ON public.emi_schedule;
CREATE TRIGGER trg_emi_calc_fields
  BEFORE INSERT OR UPDATE ON public.emi_schedule
  FOR EACH ROW EXECUTE FUNCTION fn_emi_calc_fields();
 
-- 8c: Apply updated_at triggers to all new tables
DO $$
DECLARE t TEXT;
BEGIN
...
-- ============================================================
-- STEP 10: DAILY CRON QUERY
-- Run this every day via pg_cron or your Node.js scheduler
-- Updates overdue_days + status for all pending EMIs
-- ============================================================
 
-- Save this as a stored function to call from cron:
CREATE OR REPLACE FUNCTION fn_refresh_emi_overdue_status()
RETURNS void AS $$
BEGIN
  -- Mark pending EMIs as overdue if due_date has passed
  ...
  -- Reset overdue_days to 0 for paid/waived EMIs
  UPDATE public.emi_schedule
  SET
    overdue_days = 0,
    updated_at   = NOW()
...
-- Call it like this from Node.js cron (node-cron):
-- SELECT fn_refresh_emi_overdue_status();